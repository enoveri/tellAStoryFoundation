...
import type { AboutData } from "@/lib/about-store";
...
function TabAbout({ data }: AboutPageClientProps) {
  return (
    <div className="space-y-5">
      <section className="space-y-3 px-4">
        ...
        {data.pillars.map(({ title, body }, i) => {
          ...
          return (
            <div
              key={title}
              className="flex gap-4 rounded-2xl border border-[color:var(--border)] bg-[color:var(--card)] p-4 shadow-sm"
            >
      ...
      <section className="space-y-3 px-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          {data.team.map(({ id, name, role, img }) => (
            <div
              key={id}
              className="flex flex-col items-center gap-2 rounded-2xl border border-[color:var(--border)] bg-[color:var(--card)] p-3 text-center shadow-sm"
            >
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src={img}
                alt={name}
                className="h-14 w-14 rounded-full border-2 border-[color:var(--primary-mid)] object-cover"
              />
              <p className="text-xs font-semibold leading-tight text-[color:var(--foreground)]">
                {name}
              </p>
              <p className="text-[10px] leading-tight text-[color:var(--muted)]">
                {role}
              </p>
            </div>
          ))}
      ...
      <section
        className="mx-4 rounded-2xl px-5 py-5 text-center shadow-md"
        style={{ background: "var(--inverse)" }}
      >
        <p className="mb-1 font-bold" style={{ color: "var(--inverse-fg)" }}>
          Get in touch
        </p>
        <p className="mb-4 text-sm" style={{ color: "var(--inverse-muted)" }}>
          Partner with us, share your story, or support our mission.
        </p>
        <a
          href="mailto:hello@tellastory.org"
          className="inline-flex items-center gap-2 rounded-full bg-[color:var(--inverse-fg)] px-5 py-2 text-sm font-semibold text-[color:var(--inverse)] hover:bg-[color:var(--primary-subtle)]"
        >
...
function TabGallery({ data }: AboutPageClientProps) {
  return (
    <div className="space-y-4 px-4">
      <p className="text-sm text-[color:var(--muted)]">
        Moments from our workshops, story circles, and community events.
      </p>
      ...
      <Link
        href="/blog"
        className="inline-flex items-center gap-1 text-sm font-semibold text-[color:var(--primary)] hover:underline"
      >
...
function TabPartnership({ data }: AboutPageClientProps) {
  return (
    <div className="space-y-5">
      <section className="space-y-3 px-4">
        <h3 className="text-xs font-semibold uppercase tracking-wider text-[color:var(--muted)]">
      ...
      <section className="space-y-3 px-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          {data.partners.map(({ id, name, kind, logo }) => (
            <div
              key={id}
              className="flex items-center gap-3 rounded-2xl border border-[color:var(--border)] bg-[color:var(--card)] p-3 shadow-sm"
            >
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src={logo}
      ...
      <section className="mx-4 rounded-2xl bg-gradient-to-br from-emerald-600 to-teal-700 px-5 py-5 text-white shadow-md">
        ...
        <a
          href="mailto:partnerships@tellastory.org"
          className="inline-flex items-center gap-2 rounded-full bg-white px-5 py-2 text-sm font-semibold text-[color:var(--success-text)] hover:bg-[color:var(--success-subtle)]"
        >
  ...
  return (
    <MobileShell title="About Us" subtitle="Tell A Story Foundation">
      ...
      <section className="bg-gradient-to-br from-sky-100 via-cyan-50 to-white px-5 py-6 text-center">
        <h2 className="mb-1 text-xl font-bold text-[color:var(--foreground)]">
          {data.heroTitle}
        </h2>
      ...
      <div key={active} className="animate-fade-in py-4">
        {active === "about" && <TabAbout data={data} />}
        {active === "gallery" && <TabGallery data={data} />}
        {active === "partnership" && <TabPartnership data={data} />}
      </div>
    </MobileShell>
  );
}
