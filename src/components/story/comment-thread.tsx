            ...
            return (
              <article key={comment.id} className="space-y-2 rounded-2xl border border-sky-100 bg-white p-3">
                <div className="flex items-center gap-2">
                  ...
                  />
                  <span className="text-sm font-semibold text-[color:var(--foreground)]">{user?.name ?? "Anonymous"}</span>
                  <span className="text-xs text-[color:var(--muted)]">{comment.createdAt}</span>
                </div>
                <p className="text-sm text-[color:var(--foreground)]/90">{comment.content}</p>
                <div className="space-y-2">
                  {comment.replies.map((reply) => (
                    <ReplyItem key={reply.id} reply={reply} user={findUser(reply.userId)} />
                  ))}
                </div>
              </article>
            );
