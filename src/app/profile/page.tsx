      ...
      <div className="space-y-5">
        ...
        >
          <div className="flex items-start gap-4">
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-lg font-bold" style={{ color: "var(--inverse-fg)" }}>{user.name}</p>
              {user.bio ? <p className="text-xs mt-0.5" style={{ color: "var(--inverse-muted)" }}>{user.bio}</p> : null}
              <div className="mt-2">
          ...
          <div className="mt-5 grid grid-cols-3 gap-2">
            {[
              ...
              <div key={label} className="flex flex-col items-center rounded-2xl py-3 px-2" style={{ background: "var(--inverse-border)" }}>
                <Icon size={14} style={{ color: "var(--inverse-muted)" }} />
                <span className="mt-1 text-xl font-extrabold" style={{ color: "var(--inverse-fg)" }}>{value}</span>
                <span className="text-[10px]" style={{ color: "var(--inverse-muted)" }}>{label}</span>
              </div>
            ))}
          </div>
          ...
          <div className="mt-4 flex gap-2">
            ...
            <button
              ...
              style={{ borderColor: "var(--inverse-border)", color: "var(--inverse-fg)" }}
              onClick={() => alert("Sign-out will work once Google Auth is connected.")}
            >
              <LogOut size={14} />
            </button>
          </div>
        ...
        {isAdmin && (
          <section className="px-4">
            ...
            >
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl" style={{ background: "var(--primary-dark)" }}>
                <ShieldCheck size={20} />
              </div>
              <div className="min-w-0 flex-1">
                <p className="font-semibold">Admin Dashboard</p>
                <p className="text-xs opacity-80">Manage users, blogs & platform content</p>
              </div>
        ...
        <section className="space-y-3 px-4">
          <div className="flex items-center justify-between">
            <h2 className="text-base font-semibold" style={{ color: "var(--foreground)" }}>My Stories</h2>
            <Link href="/write" className="text-sm font-medium" style={{ color: "var(--primary)" }}>
              + Write new
            </Link>
          </div>
          {myStories.length > 0
            ...
            : (
              <div className="rounded-2xl border border-dashed p-6 text-center text-sm" style={{ borderColor: "var(--border)", color: "var(--muted)" }}>
                You haven&apos;t shared a story yet.{" "}
                <Link href="/write" className="font-semibold underline" style={{ color: "var(--primary)" }}>Write your first</Link>
              </div>
            )
          }
        </section>
