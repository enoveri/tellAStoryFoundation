...
export default async function StoryDetailPage({ params }: StoryDetailPageProps) {
  ...
  return (
    <MobileShell title="Story" subtitle={author ? `By ${author.name}` : "By community member"}>

      {/* Back */}
      <div className="px-1">
        <Link href="/feed" className="inline-flex items-center gap-1 text-sm font-medium" style={{ color: "var(--primary)" }}>
      ...
      <article className="space-y-4 rounded-2xl border border-[color:var(--border)] bg-[color:var(--card)] p-4 shadow-sm">
        ...
        {author && (
          <div className="flex items-center gap-3 border-b border-[color:var(--border)] pb-4">
            <Image src={author.avatar} alt={author.name} width={36} height={36} className="shrink-0 rounded-full" />
            <div>
              <p className="text-sm font-semibold text-[color:var(--foreground)]">{author.name}</p>
              {author.bio && <p className="text-xs text-[color:var(--muted)]">{author.bio}</p>}
            </div>
        ...
        {/* Body */}
        <p className="whitespace-pre-line text-sm leading-7 text-[color:var(--foreground)]/90">{story.body}</p>
      ...
      <section className="space-y-3">
        <h3 className="text-base font-semibold text-[color:var(--foreground)]">
          Conversation <span className="text-sm font-normal text-[color:var(--muted)]">({story.comments.length})</span>
        </h3>
        <CommentComposer />
